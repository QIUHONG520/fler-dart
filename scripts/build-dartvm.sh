...
# compressed-pointers 变体：默认 compressed；--no-compressed-pointers 或
# FLER_NO_COMPRESSED_POINTERS=1 → no_cptr。环境变量用于 CI，避免旧版 workflow
# 在多级 checkout 路径下把该参数误传给了错误脚本。
COMPRESSED_PTRS="ON"
if [[ "${FLER_NO_COMPRESSED_POINTERS:-0}" == "1" || "${DART_NO_COMPRESSED_PTRS:-0}" == "1" ]]; then
    COMPRESSED_PTRS="OFF"
fi
